...
class FiscalTest extends TestCase
{
    ...
    public function testSanitizeFiscalItems(): void
    {
        ...
        $mockHttp->expects($this->once())
            ...
            ->with(
                'POST',
                'https://api.pairus.com.br/v2/fiscal/sanitize',
                $this->anything(),
                $this->callback(function ($body) {
                    $json = json_decode($body, true);
                    // A API exige {"itens": [...]} com NCM/CEST em maiúsculas (a 1.4 enviava lista crua)
                    return $json === ['itens' => [[
                        'NCM' => '00000000',
                ...
                body: json_encode([
                    ...
                    'itens' => [
                        [
                            'id' => '7891000100103',
                            'xProd' => 'LEITE CONDENSADO MOCA 395G',
                            'NCM_informado' => '00000000',
                            'CEST_informado' => null,
                            'auditoria_fiscal' => ['ncm_vigente' => false, 'alertas_fiscais' => ['NCM inativo']]
                        ]
                    ]
                ])
            ));
        ...
        $this->assertCount(1, $res['itens']);
        $this->assertFalse($res['itens'][0]['auditoria_fiscal']['ncm_vigente']);
    }
}
